 ...
 */
object ExtractRasterUDFs extends Rule[LogicalPlan] with PredicateHelper {

  private type EvalType = Int
  private type EvalTypeChecker = EvalType => Boolean

  private def hasScalarPythonUDF(e: Expression): Boolean = {
    e.find(PythonRasterUDF.isScalarPythonUDF).isDefined
  }

  private def canEvaluateInPython(e: PythonRasterUDF): Boolean = {
    e.children match {
      // single PythonUDF child could be chained and evaluated in Python
      case Seq(u: PythonRasterUDF) => e.evalType == u.evalType && canEvaluateInPython(u)
      // Python UDF can't be evaluated directly in JVM
      case children => !children.exists(hasScalarPythonUDF)
    }
  }

  private def collectEvaluableUDFsFromExpressions(expressions: Seq[Expression]): Seq[PythonRasterUDF] = {
    ...
    def collectEvaluableUDFs(expr: Expression): Seq[PythonRasterUDF] = expr match {
      case udf: PythonRasterUDF if PythonRasterUDF.isScalarPythonUDF(udf) && canEvaluateInPython(udf)
  ...
  private def extract(plan: LogicalPlan): LogicalPlan = {
    ...
    if (udfs.isEmpty) {
      ...
      val attributeMap = mutable.HashMap[PythonRasterUDF, Expression]()
      ...
      val newChildren = splitFilter.children.map { child =>
        ...
        if (validUdfs.nonEmpty) {
          require(
            validUdfs.forall(PythonRasterUDF.isScalarPythonUDF),
          ...
          val evaluation = validUdfs.partition(
            ...
            case (vectorizedUdfs, plainUdfs) if plainUdfs.isEmpty =>
              RasterEvalPython(vectorizedUdfs, child.output ++ resultAttrs, child)
      ...
      udfs.filterNot(attributeMap.contains).foreach { udf =>
        sys.error(s"Invalid PythonRasterUDF $udf, requires attributes from more than one child.")
      }

      val rewritten = splitFilter.withNewChildren(newChildren).transformExpressions {
        case p: PythonRasterUDF if attributeMap.contains(p) =>
          attributeMap(p)
      }
