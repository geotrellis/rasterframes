...
import org.locationtech.rasterframes.datasource.tensor._
import org.locationtech.rasterframes.model.TileDimensions
import org.locationtech.rasterframes.ref.RasterRef.RasterRefTile
import org.locationtech.rasterframes.util._

class RSExperiments extends TestEnvironment with TestData {
  ...
  describe("RasterSource as relation reading") {
    val DEFAULT_COLUMN_NAME = "DEFAULT"
    val b = DEFAULT_COLUMN_NAME
    println(s"default column name: ${b}")
    it("should read a multiple files with one band") {
      ...
      val head = df.head
      println("Before select", schema, head)
      println(df.count, df.show)
      df.select($"${b}_path").distinct().count() should be(3)
      df.schema.size should be(2)
    }

    // it("should default to a single band schema") {
    //   val df = spark.read.tensor.load(l8B1SamplePath.toASCIIString)
